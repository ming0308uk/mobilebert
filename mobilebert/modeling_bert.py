...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if hasattr(self, "embedding_transformation"):
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        if config.embedding_size != config.hidden_size:
            self.projection_layer = nn.Linear(config.hidden_size, config.embedding_size)
...
NORM2FN = {"layer_norm": nn.LayerNorm, "no_norm": NoNorm, "rms_norm": RMSNorm}
